...
use crate::services::{
    AuthService, EnvironmentService, InstanceService, MockService, PostgresEnvironmentRepository,
    PostgresInstanceRepository, PostgresMockRepository,
};
...
pub fn create_routes() -> Router<AppState> {
    Router::new()
        ...
        .route(
            "/api/v1/instances",
            axum::routing::get(list_instances).post(register_instance),
        )
        .route(
            "/api/v1/instances/:id",
        ...
        .route(
            ...
            axum::routing::get(get_analytics_stats),
        )
        .route(
...
pub async fn sync_pull(
    ...
    match service
        .find_modified_since(since.unwrap_or(chrono::DateTime::UNIX_EPOCH), filter)
        .await
    {
        ...
        for config_value in configs {
                ...
                serde_json::from_value::<crate::models::MockConfiguration>(config_value.clone())
            {
                match service.get(config.id).await {
                    ...
                    Err(_) => match service.save(&config).await {
                        Ok(()) => accepted.push(config.id),
...
pub async fn resolve_conflict(
    ...
    let resolution = request.get("resolution").cloned().unwrap_or(json!({}));
    ...
    let config = match strategy {
        "keep_local" => {
            ...
            let mut config: crate::models::MockConfiguration =
                serde_json::from_value(local.clone())?;
            config.version_vector.increment(Uuid::new_v4());
            service.save(&config).await?;
            config
        }
        "keep_central" => service.get(id).await?,
        "merge" => {
...
pub async fn import_mocks(
    ...
    for config_value in configs {
        match serde_json::from_value::<crate::models::MockConfiguration>(config_value) {
            Ok(config) => match service.save(&config).await {
                Ok(()) => imported += 1,
                Err(_) => skipped += 1,
            },
            Err(e) => {
...
pub async fn get_analytics_stats(State(state): State<AppState>) -> impl IntoResponse {
...
pub async fn get_mock_analytics(State(state): State<AppState>, Path(id): Path<Uuid>) -> Response {
    let exists: bool =
        sqlx::query_scalar("SELECT EXISTS(SELECT 1 FROM mock_configurations WHERE id = $1)")
            .bind(id)
            .fetch_one(&state.pool)
            .await
            .unwrap_or(false);
    ...
    let total_requests: i64 =
        sqlx::query_scalar("SELECT COUNT(*) FROM analytics_records WHERE mock_id = $1")
            .bind(id)
            .fetch_one(&state.pool)
            .await
            .unwrap_or(0);
