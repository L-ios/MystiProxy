...
mod router_tests {
    ...
    fn test_e2e_router_full_match_exact() {
        ...
        let route = Route::new(
            "/api/users".to_string(),
            MatchMode::Full,
            make_location("/api/users", MatchMode::Full),
        )
        .unwrap();
    ...
    fn test_e2e_router_prefix_match() {
        ...
        let route = Route::new(
            "/api".to_string(),
            MatchMode::Prefix,
            make_location("/api", MatchMode::Prefix),
        )
        .unwrap();
        router.add_route(route);
    ...
    fn test_e2e_router_prefix_match_root() {
        ...
        let route = Route::new(
            "/".to_string(),
            MatchMode::Prefix,
            make_location("/", MatchMode::Prefix),
        )
        .unwrap();
        router.add_route(route);
    ...
    fn test_e2e_router_regex_match_single_param() {
        ...
        let route = Route::new(
            "/users/{id}".to_string(),
            MatchMode::Regex,
            make_location("/users/{id}", MatchMode::Regex),
        )
        .unwrap();
        router.add_route(route);
    ...
    fn test_e2e_router_regex_match_multiple_params() {
        ...
        let route = Route::new(
            ...
            MatchMode::Regex,
            make_location("/users/{user_id}/posts/{post_id}", MatchMode::Regex),
        )
        .unwrap();
        router.add_route(route);
    ...
    fn test_e2e_router_prefix_regex_match() {
        ...
        let route = Route::new(
            ...
            make_location("/api/{version}/", MatchMode::PrefixRegex),
        )
        .unwrap();
        router.add_route(route);
    ...
    fn test_e2e_router_priority_first_match_wins() {
        ...
        router.add_route(
            Route::new(
                ...
                MatchMode::Prefix,
                make_location("/api/special", MatchMode::Prefix),
            )
            .unwrap(),
        );
    ...
    fn test_e2e_router_prefix_rejects_non_matching() {
        ...
        router.add_route(
            Route::new(
                "/api/v1".to_string(),
                MatchMode::Prefix,
                make_location("/api/v1", MatchMode::Prefix),
            )
            .unwrap(),
        );
...
mod static_file_tests {
    ...
    async fn test_e2e_static_range_request() {
        ...
        let response = service
            .serve_with_range("/data.bin", Some("bytes=0-9"))
            .await
            .unwrap();
    ...
    async fn test_e2e_static_range_last_n_bytes() {
        ...
        let service = StaticFileService::new(temp.path().to_path_buf());
        let response = service
            .serve_with_range("/data.bin", Some("bytes=-10"))
            .await
            .unwrap();
...
mod context_tests {
    use mystiproxy::context::{get_engine_name, set_engine_name, thread_identity, with_engine};
