...
fn test_e2e_jwt_authenticator_rejects_wrong_secret() {
    ...
    let result = auth.authenticate(&headers).unwrap();
    assert!(!result.authenticated, "JWT with wrong secret should be rejected");
}
...
fn test_e2e_jwt_authenticator_rejects_missing_header() {
    ...
    let result = auth.authenticate(&headers).unwrap();
    assert!(!result.authenticated, "missing auth header should be rejected");
}
...
fn test_e2e_jwt_authenticator_accepts_token_without_bearer_prefix() {
    ...
    let result = auth.authenticate(&headers).unwrap();
    assert!(result.authenticated, "JWT without Bearer prefix should work");
}
...
async fn test_e2e_connection_pool_default() {
    ...
    let client = pool
        .get_or_create("tcp://127.0.0.1:8080".to_string(), Some(Duration::from_secs(30)))
        .await;
